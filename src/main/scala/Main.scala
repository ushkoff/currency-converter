...
object Main extends App {
  ...
  // Improvement proposal #1: set up websocket solution for every-day-retrieving data from API.
  // Improvement proposal #2: set up analogue for nodejs cron jobs for automated data updating (once in 24 hours).
  ...
  while (true) {
    ...
    println("======================")
    println(s"Найближчі до вас обмінники, де ви можете виконати обрано операцію з $finalCurrency:")
    println(s"https://www.google.com/maps?q=nearest+money+exchange+${finalCurrency}+to+me&sca_esv=587494963&uact=5&gs_lp=Egxnd3Mtd2l6LXNlcnAiIG5lYXJlc3QgbW9uZXkgZXhjaGFuZ2UgVVNEIHRvIG1lMgUQIRigATIFECEYoAEyBBAhGBUyCBAhGBYYHhgdMggQIRgWGB4YHTIIECEYFhgeGB1ImAZQlgFYkQVwAHgCkAEAmAG9AaAB1AaqAQMwLja4AQPIAQD4AQHCAgQQABhHwgIHECEYoAEYCuIDBBgAIEGIBgGQBgg&um=1&ie=UTF-8&sa=X&ved=2ahUKEwjKvfKr-_OCAxXS7rsIHSVxBTYQ_AUoAXoECAEQAw")
